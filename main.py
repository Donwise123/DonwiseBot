from telethon import TelegramClient, events from telethon.sessions import StringSession import os from keep_alive import keep_alive from datetime import datetime, timedelta import asyncio import requests import random import pytz from PIL import Image, ImageDraw, ImageFont
...
@client.on(events.NewMessage(chats=source_channels)) async def forward_signal(event): global daily_count, last_reset_date, morning_posted
...
async def post_weekly_summary(): wins = losses = pending = 0 for signal in signal_log: current = await get_price(signal['pair']) if current is None: pending += 1 continue if signal['action'] == 'buy': if current >= signal['tp']: wins += 1 elif current <= signal['sl']: losses += 1 else: pending += 1 elif signal['action'] == 'sell': if current <= signal['tp']: wins += 1 elif current >= signal['sl']: losses += 1 else: pending += 1
...
def schedule_weekly_summary(): async def job(): while True: now = datetime.now(nigeria_tz) if now.weekday() == 5 and now.hour == 9 and now.minute == 0: await post_weekly_summary() await asyncio.sleep(60) await asyncio.sleep(30) client.loop.create_task(job())
